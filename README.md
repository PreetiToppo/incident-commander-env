...
AI agents act as on-call engineers, triaging and resolving production incidents through systematic investigation — reading logs, checking metrics, tracing requests, and applying correct fixes. Inspired by real SRE runbooks at MAANG companies.
...
`check_logs`, `check_metrics`, `check_traces`, `check_dependencies`, `run_query`, `identify_root_cause`, `increase_connection_pool`, `restart_service`, `rolling_restart`, `fix_cache_config`, `disable_auto_commit`, `fix_idempotency_read_source`, `drain_kafka_backlog`, `reconcile_inventory`, `resolve`, `escalate`

## Observation Space

`incident_id`, `title`, `severity`, `affected_services`, `alert_messages`, `logs`, `metrics`, `runbook_hints`, `diagnosis_so_far`, `available_actions`, `current_step`, `max_steps`, `resolved`
